...
def test_ssd():
    ...
    ch_types = [['mag']*10, ['eeg']*10]
    info = create_info(ch_names=n_channels, sfreq=sf, ch_types=ch_types)
    ...
    spec_ratio, _ = get_spectral_ratio(ssd.transform(X), sf, ssd)
    ...
    R2 = r2_score(psd_out, psd_S)
    assert np.abs(R2) > 0.95
